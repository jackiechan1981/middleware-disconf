...
public class DisconfAspectJ {
		...
		if (DisClientConfig.getInstance().ENABLE_DISCONF) {
			...
			if (field != null) {
				...
				if (DisClientConfig.getInstance().ENABLE_NODE_EXISTS_CHECK) {
					...
					DisconfCoreProcessUtils.rebuildExpiredNodeIfNonWatchExecuted(disconfStoreProcessor,
							disconfFile.filename(), DisConfigTypeEnum.FILE);
				}

				Object ret = disconfStoreProcessor.getConfig(disconfFile.filename(), disconfFileItem.name());
				if (ret != null) {
					LOGGER.debug("using disconf store value: " + disconfFile.filename() + " (" + disconfFileItem.name()
							+ " , " + ret + ")");
					return ret;
				}
			}
	...
	public Object decideAccess(ProceedingJoinPoint pjp, DisconfItem disconfItem) throws Throwable {
		...
		if (DisClientConfig.getInstance().ENABLE_DISCONF) {
			...
			if (DisClientConfig.getInstance().ENABLE_NODE_EXISTS_CHECK) {
				// 解决disconf和应用断连问题：重建过期节点
				DisconfCoreProcessUtils.rebuildExpiredNodeIfNonWatchExecuted(disconfStoreProcessor, disconfItem.key(),
